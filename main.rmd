...
\begin{align*}
  &1: \delta IQ \sim N(\alpha + \beta \cdot \text{decade}, \sigma)\\
  &2: \delta IQ \sim N(\alpha + \gamma \cdot \text{schooling level}, \sigma)\\\
  &3: \delta IQ \sim N(\alpha + \beta \cdot \text{decade} + \gamma \cdot \text{schooling level}, \sigma)\\
\end{align*}
...
+ The noise, $\sigma$, is distributed normally and somewhat strictly due to us not wanting for much of the variance to be explained by noise, believing that a stricter $\sigma$ would result in us detecting bad fits for our models more quickly.
