...
model {
  ...
  mu1 ~ normal(a[1] + b[1] * x1 + c[1] * z1, sigma);
  mu2 ~ normal(a[2] + b[2] * x2 + c[2] * z2, sigma);
  mu3 ~ normal(a[3] + b[3] * x3 + c[3] * z3, sigma);
  mu4 ~ normal(a[4] + b[4] * x4 + c[4] * z4, sigma);
  mu5 ~ normal(a[5] + b[5] * x5 + c[5] * z5, sigma);
  // likelihood
  y1 ~ normal(mu1, 1); 
  y2 ~ normal(mu2, 1); 
  y3 ~ normal(mu3, 1); 
